        ...
        if (args.pluginaction === "runUpdate") {
            ...
            if (os === "win32") {
                ...
                child = cp.execFile(process.env['windir'] + '\\System32\\WindowsPowerShell\\v1.0\\powershell.exe', [
                    '-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-Command', 'try { if (-not (Get-Module -ListAvailable -Name PSWindowsUpdate)) { throw "PSWindowsUpdate module not found. Install it for all users with: Install-Module PSWindowsUpdate -Scope AllUsers"; } Import-Module PSWindowsUpdate -ErrorAction Stop; $updates = Get-WindowsUpdate -MicrosoftUpdate -IgnoreUserInput -ErrorAction SilentlyContinue; $scanSummary = if ($updates) { $updates | Select-Object KBArticleID, Title, Size, MsrcSeverity, IsDownloaded, IsInstalled | Format-Table -AutoSize | Out-String } else { "No applicable updates detected." }; $installOutput = $null; if ($updates) { $installOutput = Install-WindowsUpdate -AcceptAll -AutoReboot -IgnoreReboot -MicrosoftUpdate -IgnoreUserInput -Verbose -ErrorAction Continue *>&1; } $installText = if ($installOutput) { $installOutput | Out-String } else { "Install-WindowsUpdate not invoked because no updates were returned by Get-WindowsUpdate." }; $output = @("=== Get-WindowsUpdate ===", $scanSummary.TrimEnd(), "=== Install-WindowsUpdate ===", $installText.TrimEnd()); $output | Out-String } catch { $_ | Out-String }'
